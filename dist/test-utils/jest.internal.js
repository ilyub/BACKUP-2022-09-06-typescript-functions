...
exports.matchers = {
    executionTimeToBe: async (got, expected, precision = 10) => {
        ...
        const t2 = Date.now();
        const gotTime = t2 - t1;
        return (0, expect_1.buildResult)(Math.abs(gotTime - expected) <= precision, "Unexpected execution time", gotTime, expected);
    },
    mockCallsToBe: (got, ...expected) => {
        __1.assert.byGuard(got, expect_internal_1.isMock, "Expecting mock function");
        const result = (0, expect_1.buildResult)((0, expect_utils_1.equals)(got.mock.calls, expected), "Unexpected mock calls", got.mock.calls, expected, true);
        got.mockClear();
        return result;
    },
    toBeSameAs: (got, expected) => {
        ...
        return (0, expect_1.buildResult)(got === expected, "Object are not identical", got, expected);
    }
};
